...
if __name__ == '__main__':
    ...
    # Вывод полной таблицы после решения
    result_table = []
    for j in range(grid.m + 1):
        for i in range(grid.n + 1):
            x = i * grid.h
            y = j * grid.k
            numerical = grid.final_grid[j][i]
            if not np.isnan(numerical):
                exact = u(x, y)
    ...
    print(tabulate.tabulate(
        result_table,
        headers=['x', 'y', 'Численное решение', 'Точное решение', 'Погрешность'],
